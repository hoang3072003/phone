...
def search_phone_by_rules(
    ...
    Args:
        name: Danh sách tên điện thoại hoặc từ khóa mà bạn muốn tìm kiếm. Đặt name = "pass" nếu không muốn sử dụng bộ lọc này.
            Ví dụ:
        ...
        colors: Danh sách màu sắc của điện thoại mà bạn muốn tìm. Đặt colors = "pass" nếu không sử dụng bộ lọc này.
            Ví dụ:
                - "Black; White".
    ...
    if name != "pass":
        ...
        phones1=[]
        for a in names:
            phones = phones1+[p for p in phones if p["name"].lower()==compare(a,p['name'])]
    if colors != "pass":
        colorss = [c.strip() for c in colors.split(";")]
        phones = [p for p in phones if p["colors"] in colorss]

    if price != "pass":
        prices = [int(p.strip()) for p in price.split(";")]
        phones = [p for p in phones if int(p["price"]) >= prices[0] and int(p["price"]) <= prices[1]]
    ...
    if choose_range != "pass":
        ...
        phones = phones[start - 1 : end]

    return json.dumps({"phones": phones}, ensure_ascii=False, indent=4)
...
model = genai.GenerativeModel(
    ...
    tools=tools,
    system_instruction="""Bạn là một trợ lý ảo thông minh, làm nhiệm vụ bán hàng cho một cửa hàng điện thoại. Bạn có khả năng truy xuất thông tin từ cơ sở dữ liệu để trả lời câu hỏi của người dùng một cách chính xác và hiệu quả.
                                                    Hãy nhớ:
                                                    ...
                                                    bạn sẽ tìm danh sách những điện thoại phù hợp bằng tool 'search_phone_by_rules' sau đấy tìm thông tin của điện thoại bằng 'search_phone_info_by_name'.
                                                """,
)